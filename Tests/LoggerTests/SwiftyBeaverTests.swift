...
class LoggerTests: XCTestCase {
    ...
    func testLogVerifiesIfShouldLogOnAllDestinations() {
        ...
        XCTAssertEqual(dest1.shouldLogToLevel, Logger.Level.warning)
        XCTAssertEqual(dest2.shouldLogToLevel, Logger.Level.warning)
    ...
    func testLogCallsAllDestinations() {
        ...
        XCTAssertEqual(dest1.didSendToLevel, Logger.Level.warning)
        XCTAssertEqual(dest2.didSendToLevel, Logger.Level.warning)
    ...
    func testDestinationIntegration() {
        ...
        file2.logFileURL = URL(string: "file:///tmp/testLogger2.log")!
        console2.format = "$L: $M"
        file2.minLevel = Logger.Level.debug
    ...
    func testVersionAndBuild() {
        XCTAssertGreaterThan(Logger.version.length, 4)
        XCTAssertGreaterThan(Logger.build, 500)
    }

    func testStripParams() {
        var f = "singleParam"
        XCTAssertEqual(Logger.stripParams(function: f), "singleParam()")
        f = "logWithParamFunc(_:foo:hello:)"
        XCTAssertEqual(Logger.stripParams(function: f), "logWithParamFunc()")
        f = "aFunc()"
        XCTAssertEqual(Logger.stripParams(function: f), "aFunc()")
    }
...
private class MockDestination: BaseDestination {
    var didSendToLevel: Logger.Level?
    ...
    override func send(_ level: Logger.Level, msg: String, thread: String, file: String, function: String, line: Int, context: Any?) -> String? {
    ...
    var shouldLogToLevel: Logger.Level?
    var shouldLogPath: String?
    var shouldLogFunction: String?
    var shouldLogMessage: String?
    override func shouldLevelBeLogged(_ level: Logger.Level, path: String, function: String, message: String?) -> Bool {
